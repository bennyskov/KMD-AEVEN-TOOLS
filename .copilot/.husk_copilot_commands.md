...
please validate the git steps and the roles. I'm not sure that they works as intended.
i'd like the files which is created at runtime - cleanup_candidates_{{ ansible_date_time.date }}.txt on the server ~ nodename to be copied back to the ansible tower host where the git repository exists. i think the path for the repository is named in var script_homedir.
